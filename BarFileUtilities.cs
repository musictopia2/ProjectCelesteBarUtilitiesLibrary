namespace ProjectCelesteBarUtilitiesLibrary;
public static class BarFileUtilities
{
    private static readonly ArrayPool<byte> _bufferPool = ArrayPool<byte>.Shared;
    //i have 2 methods.  everything needed publically is here.
    public static async Task ExtractBarFilesAsync(string inputFile, string outputPath)
    {
        ...
        {
            await ExtractBarFileContents(barFileInfo, barFilesInfo.RootPath, barFileStream, outputPath, true);
        }
    }
    ...
    //i like it better this way to make it easier to use.
    private static async Task CreateBarFileAsync(string inputPath, string outputFileName, string rootdir,
        bool ignoreLastWriteTime = true)
    {
    ...
    private static async Task CreateBarFileAsync(IEnumerable<FileInfo> fileInfos, string inputPath,
    ...
    {
        using var binReader = new BinaryReader(fileStream, Encoding.UTF8, true);
        //Read Header
        binReader.BaseStream.Seek(0, SeekOrigin.Begin); //Seek to header
        var barFileHeader = new BarFileHeader(binReader);
        //Read Files Info
